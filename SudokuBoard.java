...
public class SudokuBoard extends Observable {
    ...
    public void solvePuzzle(int row, int col){
        Stack<Cell> stack = new Stack<>();// we create a new stack
        for(row=0;row<BOARD_SIZE;row++){
            for(col=0;col<BOARD_SIZE;col++){
                ...
                if(getCellValue(row,col)==0){
                    ...
                    setCellValue(row,col,(byte)(1));// set the value in the board if we do not do that it will not update the value
                    stack.push(new Cell(row,col,(byte)(1)));// this is push it into a stack

                }
                boolean temp1 =boardLegal();

                while(!temp1){

                    Cell temp  = stack.pop();// temp store the location and vlue which is wrong
                    col=temp.getCol();// get row and col at that place
                    row=temp.getRow();
                    stack.push(new Cell(row,col,(byte)((Character.getNumericValue(temp.getvalue())+1)%10)));// push the value after the value we tested
                    setCellValue(row,col,(byte)((Character.getNumericValue(temp.getvalue())+1)%10));//update the board
                    temp1 =boardLegal();
        ...
        // to update the GUI, call setCellValue(row, col, (byte) value);
        // for example:  setCellValue( 0, 0, (byte) 7 );
